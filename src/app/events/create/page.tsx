...
function CreateEventForm() {
  ...
  const toLocalDateInput = (value: string | Date | null | undefined) => {
    if (!value) return '';
  ...
  const toLocalTimeInput = (value: string | Date | null | undefined) => {
    if (!value) return '';
    const d = new Date(value);
    const local = new Date(d.getTime() - d.getTimezoneOffset() * 60000);
  ...
  const toLocalDateTimeInput = (value: string | Date | null | undefined) => {
    if (!value) return '';
    const d = new Date(value);
    const local = new Date(d.getTime() - d.getTimezoneOffset() * 60000);
